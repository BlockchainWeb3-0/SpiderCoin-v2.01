...
describe("transaction test", () => {
  describe("createTransaction function test", () => {
    ...
    beforeAll(()=>{
      ...
      sendingAmount = 20;
      privateKey = Wallet.readPrivateKeyFromWallet();
      ...
      const txOuts1 = TxOut.createTxOuts(receiverAddress, 10, Wallet.getWalletPublicKey(), 0);
      const tx1 = new Transaction("", [txIn1], txOuts1)
      tx1.id = Transaction.calTxId(tx1)
      txIn1 = TxIn.signToTxIn(txIn1, Wallet.readPrivateKeyFromWallet(), tx1.id)
      tx1.txIns = [txIn1]
      txpool.push(tx1);
